...
class DataConfig:
    ...
    hist_sequence_keys: Sequence[str] = ("image", "wrist_image")
...
class ModelTransformFactory(GroupFactory):
    ...
    def __call__(self, model_config: _model.BaseModelConfig) -> _transforms.Group:
        match model_config.model_type:
            ...
            case _model.ModelType.PI05Rtc:
                assert isinstance(model_config, pi0_config.Pi0RtcConfig)
                return _transforms.Group(
                    inputs=[
                        _transforms.InjectDefaultPrompt(self.default_prompt),
                        _transforms.ResizeImages(224, 224),
...
class LeRobotCalvinDataConfig(DataConfigFactory):
    """
    Config for training on CALVIN (InternData-Calvin_ABC) in LeRobot format.
    Ported from the FASTER pi05_calvin config, with support for history-stacked
    image frames (stream-style training) via `hist_sequence_keys`.
    """

    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        # The repack transform maps the dataset keys (as defined in the InternData-Calvin_ABC
        # LeRobot features) to the keys expected by `calvin_policy.CalvinInputs`.
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
                    {
        ...
        # History frames are stacked on the raw dataset keys (before repacking).
        hist_sequence_keys: Sequence[str] = ("video.image_base", "video.image_wrist")
        ...
        return dataclasses.replace(
            ...
            hist_sequence_keys=hist_sequence_keys,
            # Match the FASTER pi05_calvin config: plain z-score normalization for CALVIN.
...
class LeRobotAgilexDataConfig(DataConfigFactory):
    ...
    # Action keys that will be used to read the action sequence from the dataset.
    action_sequence_keys: Sequence[str] = ("action",)
    hist_sequence_keys: Sequence[str] = ("observation.images.cam_high", "observation.images.cam_left_wrist", "observation.images.cam_right_wrist")
...
model_root = "/home/ma-user/work/model/openpi"
dataset_root = "/home/ma-user/work/users/jhhou/dataset/real_world_data_cb408"
...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi05_libero",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=10, discrete_state_input=False, hist_horizon=1),
        data=LeRobotLiberoDataConfig(
            repo_id="physical-intelligence/libero",
            base_config=DataConfig(prompt_from_task=True, hist_horizon=1),
    ...
    TrainConfig(
        name="pi05_libero_stream3",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=10, discrete_state_input=False, hist_horizon=3),
        data=LeRobotLiberoDataConfig(
            repo_id="physical-intelligence/libero",
            base_config=DataConfig(prompt_from_task=True, hist_horizon=3, enable_jitter=False),
            extra_delta_transform=False,
            assets=AssetsConfig(assets_dir="assets/pi05_libero", asset_id="physical-intelligence/libero"),
        ),
        batch_size=256,
    ...
    TrainConfig(
        name="pi05_libero_stream5",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=10, discrete_state_input=False, hist_horizon=5),
        data=LeRobotLiberoDataConfig(
            repo_id="physical-intelligence/libero",
            base_config=DataConfig(prompt_from_task=True, hist_horizon=5, enable_jitter=False),
            extra_delta_transform=False,
            assets=AssetsConfig(assets_dir="assets/pi05_libero", asset_id="physical-intelligence/libero"),
        ),
        batch_size=256,
    ...
    #
    TrainConfig(
        name="pi05_calvin_stream5",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=10, discrete_state_input=False, hist_horizon=5),
        data=LeRobotCalvinDataConfig(
            repo_id="InternRobotics/InternData-Calvin_ABC",
            base_config=DataConfig(prompt_from_task=True, hist_horizon=5, hist_interval=5, enable_jitter=False),
            assets=AssetsConfig(
                assets_dir="assets/pi05_calvin",
                asset_id="InternRobotics/InternData-Calvin_ABC",
            ),
        ),
        batch_size=256,
        lr_schedule=_optimizer.CosineDecaySchedule(
    ...
    TrainConfig(
        name="pi05_calvin_stream3",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=10, discrete_state_input=False, hist_horizon=3),
        data=LeRobotCalvinDataConfig(
            repo_id="InternRobotics/InternData-Calvin_ABC",
            base_config=DataConfig(prompt_from_task=True, hist_horizon=3, hist_interval=5, enable_jitter=False),
            assets=AssetsConfig(
                assets_dir="assets/pi05_calvin",
                asset_id="InternRobotics/InternData-Calvin_ABC",
            ),
    ...
    #
    TrainConfig(
        name="pi05_stream5_aloha_shell_game",
        model=pi0_config.Pi0Config(pi05=True, hist_horizon=5),
